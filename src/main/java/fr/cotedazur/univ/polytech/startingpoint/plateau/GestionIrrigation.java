...
public class GestionIrrigation {
            ...
            for (Parcelle parcelle : parcellesPossiblePourIrrigation) {
                if (parcelle.getClass() == ParcelleCouleur.class) {
                    ...
                    for (Position position : positionsIrrigation) {
                        Optional<Irrigation> irrigationDejaPosee = chercheIrrigation(position, parcelleAIrrigee.getPosition(), irrigationsPosees);
    ...
    public static Optional<Set<Irrigation>> checkIrrigationsAutour(@NotNull Map<Parcelle, Parcelle[]> parcellesEtVoisines,
                                              ParcelleCouleur parcelleCouleur, @NotNull Set<Irrigation> irrigationsDisponibles,
        ...
        try {
            ...
            for (int i = 0; i < voisins.length - 1; i++) {
                ...
                Optional<Irrigation> irrigation = chercheIrrigation(voisins[i].getPosition(), voisins[i + 1].getPosition(), irrigationsPosees);

                // si il en existe une, on met à jour le set d'irrigations disponible à partir de cette irrigation
                if (irrigation.isPresent() && irrigationsDisponiblesSet.isPresent()) irrigationsDisponiblesSet = addIrrigationDisponible(parcellesEtVoisines, irrigation.get(), irrigationsDisponiblesSet.get(), irrigationsPosees);
                if (voisins[i].equals(GestionParcelles.ETANG)) voisinEtang = true;
            }
            Optional<Irrigation> irrigation = chercheIrrigation(voisins[5].getPosition(), voisins[0].getPosition(), irrigationsPosees);
            if (irrigation.isPresent() && irrigationsDisponiblesSet.isPresent()) irrigationsDisponiblesSet = addIrrigationDisponible(parcellesEtVoisines, irrigation.get(), irrigationsDisponiblesSet.get(), irrigationsPosees);
                    ...
                    if (parcelle.getClass() == ParcelleCouleur.class) {
                        ...
                        for (Parcelle voisin : voisinsParcelle) {
                            if (voisin.equals(GestionParcelles.ETANG)) {
                                ...
                                if (irrigationsDisponiblesSet.isPresent()) irrigationsDisponiblesSet.get().add(irrigationDispo);
                            }
                        }
     ...
     */
    public static Optional<Irrigation> chercheIrrigation(Position position1, Position position2, @NotNull Set<Irrigation> irrigationsPosees){
